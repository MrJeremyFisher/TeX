...
\begin{soln}
  ...
  we cannot factor out a linear factor. So this must factor (if it is reducible) as an irreducible cubic and an irreducible quadratic, both of which are monic by the theorem referenced in the notes. So,
  $$f(x)=(x^3+bx^2+cx+d)(x^2+fx+g).$$
  Multiplying this out we obtain that
  $$f(x)=x^5 +3x^2 +1=x^5+(b + f)x^4  + (bf + c + g)x^3 + (cf + bg + d)x^2  + (df + cg)x+ dg$$
  ...
  \begin{align*}
    b       & =-f             \\
    c       & =f^2-g          \\
    c       & =-\frac{3-f^2g-g^2+gf^2}{f^2} 
  \end{align*}
  which cannot be the case as $\frac{3-f^2g-g^2+gf^2}{f^2}\neq f^2-g$ and hence $f(x)$ cannot be reduced over $\mathbb{Z}$ and therefore cannot be reduced over $\mathbb{Q}$.
\end{soln}
...
\begin{soln}
  \begin{enumerate}[label=(\alph*)]
    \item A ring homomorphism is a map which preserves both addition and multiplication. I'll start with addition,
          ...
          \begin{align*}
            \sum_{i=0}^{k}\left[\left(a_i+_nb_i\right)\mod n\right] x^i & =\sum_{i=0}^{k}\left[\left(a_i\mod n+b_i\mod n\right)\mod n\right] x^i                                                                                                 \\
                                                                        ...
                                                                        & =\left(\sum_{i=0}^{k}\left(a_i\mod n\right)x^i+\sum_{i=0}^{k}\left(b_i\mod n\right)x^i\right)\mod n\justif{\quad}{$\mod n$ because this is coefficient addition $+_n$} \\
          ...
          \begin{align*}
            \phi_n\left(\sum_{i=0}^{k}a_ix^i\sum_{i=0}^{k}b_ix^i\right) & = \phi_n\left(\sum_{i=0}^{2k}\left[x^i\sum_{m+l=i}a_mb_l\right]\right)                                     \\
                                                                        ...
                                                                        & = \left(\left[\sum_{i=0}^{k}x^ia_i\mod n\right]\cdot\left[\sum_{i=0}^{k}x^ib_i\mod n\right]  \right)\mod n \\
                                                                        & =\phi_n\left(\sum_{i=0}^{k}a_ix^i\right)\phi_n\left(\sum_{i=0}^{k}b_ix^i\right)
          \end{align*}
          This can also be seen (and was most immediately obvious to me) by noting that we already know that the ``mod map'' from $\mathbb{Z}\to\mathbb{Z}_n$ is a ring homomorphism
          and hence the given map, which really only acts on the coefficients, will naturally be one as well.
    ...
    \item \begin{enumerate}[label=(\roman*)]
            ...
            \item Here we take $p=2$ so that $\phi_2(21x^3 - 3x^2 + 2x + 9)=x^3 + x^2 + 1$. Since this has no roots in $\mathbb{Z}_2$ it cannot be factored and hence cannot be reduced.
            \item Here we take $p=2$ so that $\phi_2(x^4 + 7x^3 + 16x^2 + 8x + 11)=x^4 + x^3 + 1$ which has no roots in $\mathbb{Z}_2$ and because it does not factor as the square of the only irreducible quadratic
                  (as I show in the next question because I did these out of order), it does not reduce and hence neither does the original polynomial.
            \item Here we take $p=7$ so that $\phi_7(3x^4 + 9x^3 + 6x^2 + 3x + 1)=3x^4 + 2x^3 + 6x^2 + 3x + 1$ and since $\phi_2(3x^4 + 2x^3 + 6x^2 + 3x + 1)=x^4 + x + 1$ and since this has no zeroes
                  in $\mathbb{Z}_2$ if it factors it must be as two irreducible quadratics. The only such quadratic in $\mathbb{Z}_2$ is $x^2+x+1$ and
                  $(x^2+x+1)^2= x^4+x^3+x^2+x^3+x^2+x+x^2+x+1=x^4+2x^3+3x^2+2x+1\neq x^4 + x + 1$ and hence, going up the stack, our orignal polynomial is irreducible.
            \item Here we take $p=3$ so that $\phi_3(x^5 + 2x + 4)=x^5+2x+1$ which does not have zeroes in $\mathbb{Z}_3$ and hence has no linear factors. It must therefore factor into a cubic
                  and a quadratic. Now, I think the best way to do this is to attempt to divide out every irreducible quadratic in $\mathbb{Z}_3$
                  because doing so would yield the cubic we could factor with. They must be irreducible because we have no linear factors.
                  These are $x^2+x+2$, $x^2+2x+2$, and $x^2+1$. So,
                  \begin{equation}
                    \polylongdiv{x^5+2x+1}{x^2+x+2}
                  \end{equation}
                  and
                  \begin{equation}
                    \polylongdiv{x^5+2x+1}{x^2+2x+2}
                  \end{equation}
                  and
                  \begin{equation}
                    \polylongdiv{x^5+2x+1}{x^2+1}
                  \end{equation}
                  and since none of these remainders are zero $\mod 3$ we can say the polynomial $x^5+2x+1$ does not reduce in $\mathbb{Z}_3$ and hence the original polynomial is irreducible over $\mathbb{Q}$.
          \end{enumerate}
  \end{enumerate}
\end{soln}
...
\begin{soln}
  \begin{enumerate}[label=(\alph*)]
    ...
    \item Since $\rho(x^3+2x^2+3)=3x^3+2x+1$ and this is irreducible since it has no zeroes in $\mathbb{Z}_5$. Indeed, since $x^3+2x^2+3$ also has no zeroes in $\mathbb{Z}_5$
          we've verified the result for this case.
