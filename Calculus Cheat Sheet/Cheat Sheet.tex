...
$d\bv{l}=dx\ux+dy\ux+dz\uz$, $d\tau=dxdydz$\\
\subsubsection*{Gradient}
\begin{align*}
        \grad t =\partial_xt\ux+\partial_yt\uy+\partial_zt\uz
\end{align*}
\subsubsection*{Divergence}
\begin{align*}
        \grad\cdot \bv{v}=\partial_x v_x\ux+\partial_yv_y\uy+\partial_zv_z\uz
\end{align*}
\subsubsection*{Curl}
\begin{flalign*}
        \grad\times \bv{v} & =\left(\partial_y v_z-\partial_z v_y\right)\ux \\
                           & +\left(\partial_zv_x-\partial_x v_z\right)\uy  \\
                           & +\left(\partial_xv_y-\partial_yv_x\right)\uz
\end{flalign*}
\subsubsection*{Laplacian}
\begin{align*}
        \grad^2t=\partial_{xx}t+\partial_{yy}t+\partial_{zz}t
\end{align*}
\newpage
% Spherical
\subsection*{Spherical}
$d\bv{l}=dr\uv{r}+rd\theta\uv{\theta}+r\sin\theta d\phi\uv{\phi}$, $d\tau=r^2\sin\theta drd\theta d\phi$
\subsubsection*{Gradient}
\begin{align*}
        \grad t =\partial_rt\uv{r}+\frac{1}{r}\partial_\theta t\uv{\theta}+\frac{1}{r\sin\theta}\partial_\phi t\uv{\phi}
\end{align*}
\subsubsection*{Divergence}
\begin{align*}
        \grad\cdot \bv{v} & =\frac{1}{r^2}\partial_r(r^2v_r)                           \\
                          & +\frac{1}{r\sin\theta}\partial_\theta(\sin\theta v_\theta) \\
                          & +\frac{1}{r\sin\theta}\partial_\phi v_\phi
\end{align*}
\subsubsection*{Curl}
\begin{flalign*}
        \grad\times \bv{v} & =\frac{1}{r\sin\theta}\left[\partial_\theta(\sin\theta v_\phi) -\partial_\phi v_\theta\right]\uv{r} \\
...
\subsubsection*{Laplacian}
\begin{align*}
        \grad^2t & =\frac{1}{r^2}\partial_r(r^2\partial_r t)                            \\
                 & +\frac{1}{r^2\sin\theta}\partial_\theta(\sin\theta\partial_\theta t) \\
                 & +\frac{1}{r^2\sin^2\theta}\partial_{\phi\phi}t
\end{align*}
% Cylindrical
\subsection*{Cylindrical}
$d\bv{l}=ds\uv{s}+sd\phi\uv{\phi}+dz\uz$, $d\tau=sds d\phi dz$
\subsubsection*{Gradient}
\begin{align*}
        \grad t =\partial_st\uv{s}+\frac{1}{s}\partial_\phi t\uv{\phi}+\partial_z t\uz
\end{align*}
\subsubsection*{Divergence}
\begin{align*}
        \grad\cdot \bv{v} & =\frac{1}{s}\partial_s(sv_s)                      \\
                          & +\frac{1}{s}\partial_\phi(v_\phi) +\partial_z v_z
\end{align*}
\subsubsection*{Curl}
\begin{flalign*}
        \grad\times \bv{v} & =\left[\frac{1}{s}\partial_\phi v_z-\partial_z v_\phi\right]\uv{s} \\
                           & +\left[\partial_zv_s-\partial_sv_z\right]\uv{\phi}                 \\
                           & +\frac{1}{s}\left[\partial_s(sv_\phi)-\partial_\phi v_s\right]\uz
\end{flalign*}
\subsubsection*{Laplacian}
\begin{align*}
        \grad^2t & =\frac{1}{s}\partial_s(s\partial_s t)+\frac{1}{s^2}\partial_{\phi\phi}t+\partial_{zz}t
\end{align*}
\subsection*{Vector Identities}
$\bv{A}\cdot(\bv{B}\times \bv{C})=\bv{B}\cdot(\bv{C}\times \bv{A})=\bv{C}\cdot(\bv{A}\times \bv{B})$\\
$\bv{A}\times(\bv{B}\times \bv{C})=\bv{B}(\bv{A}\cdot\bv{C})=\bv{C}(\bv{A}\cdot\bv{B})$\\
etc.. will fill in later
\subsection*{Theorems}
\subsubsection*{Gradient Theorem}
$$\int_{\bv{a}}^{\bv{b}}(\grad f)\cdot d\bv{l}=f(\bv{b})=f(\bv{a})$$
\subsubsection*{Divergence Theorem}
$$\int_\mathcal{V}\grad\cdot \bv{A}d\tau=\oint_\mathcal{S}\bv{A}\cdot d\bv{a}$$
\subsubsection*{Curl (Stokes') Theorem}
$$\int_\mathcal{S}(\grad\times \bv{A})\cdot d\bv{a}=\oint_\mathcal{C}\bv{A}\cdot d\bv{l}$$
...
$
        ...
        \end{cases}
        \begin{cases}
                \ux & =\sin\theta\cos\phi\uv{r}               \\
                    & +\cos\theta\cos\phi\uv{\theta}          \\
                    & -\sin\phi\uv{\phi}                      \\
                \uy & =\sin\theta\sin\phi\uv{r}               \\
                    & +\cos\theta\sin\phi\uv{\theta}          \\
                    & +\cos\phi\uv{\phi}                      \\
                \uz & =\cos\theta\uv{r}-\sin\theta\uv{\theta}
        \end{cases}
$
$
        \begin{cases}
        ...
        \end{cases}
        \begin{cases}
                \uv{r}      & =\sin\theta\cos\phi\ux \\
                            & +\sin\theta\sin\phi\uy \\
                            & +\cos\theta\uz         \\
                \uv{\theta} & =\cos\theta\cos\phi\ux \\
                            & +\cos\theta\sin\phi\uy \\
                            & -\sin\theta\uz         \\
                \uv{\phi}   & =-\sin\phi\ux          \\
                            & +\cos\phi\uy
...
\end{cases}
\begin{cases}
        \ux&=\cos\phi\uv{s}-\sin\phi\uv{\phi}\\
        \uy&=\sin\phi\uv{s}+\cos\phi\uv{\phi}\\
        \uv{z}&=\uv{z}
\end{cases}
$
...
\end{cases}
\begin{cases}
        \uv{s}&=\cos\phi\ux+\sin\phi\uy\\
        \uv{\phi}&=-\sin\phi\ux+\cos\phi\uy\\
        \uv{z}&=\uv{z}
\end{cases}
$
