...
\begin{soln}
  \begin{enumerate}[label=(\alph*)]
    \item The field of a current carrying wire is entirely in the $\uv{\phi}$ direction in cylindrical coordinates. Then since
          ...
          $s$.
    \item Taking a contour of length $\ell$ which slices the outer conductor such that there is maximum flux
          through the contour
          \begin{figure}[h]
            \centering
            \begin{tikzpicture}
              ...
              \draw[red!80!black, ->-=.5] (-\R,0) -- (\R,0) node[midway, below]{$I$};
              \draw[blue!80!black, ->-=.5] (\R,1) -- (-\R,1) node[midway, above]{$I$};
              \draw[blue!80!black, ->-=.5] (\R,-1) -- (-\R,-1) node[midway, below]{$I$};
              \draw[-Latex] (-1.75, 0) -- (-1.75,1) node[midway, left]{$a$};
              \draw[-Latex] (0, 0) -- (0,0.3) node[midway, right]{$s$};

              \draw[] (-1,0.3) rectangle (1,1.5);
              \draw[Latex-Latex] (-1, 1.65) -- (1,1.65);
              \node[above, fill=white, rounded corners=2pt, inner sep=1pt] at (0,1.5) {$\ell$};

              \draw[<-] (\R+0.1,1) to[out=0, in=180] (3,0) node[right]{Outer conductor};
              \draw[<-] (\R+0.1,-1) to[out=0, in=180] (3,0);
              \draw[<-] (-\R-0.1,0) to[out=180, in=0] (-3,0) node[left]{Inner wire};
            \end{tikzpicture}
            \caption{The setup and contour (in black)}
          \end{figure}

          we note that using our definition of flux,
