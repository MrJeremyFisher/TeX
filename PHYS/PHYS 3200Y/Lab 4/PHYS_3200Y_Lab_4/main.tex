...
\section{Theory}
  \begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.5\textwidth}
      ...
      \end{tikzpicture}
    \end{subfigure}%
    \caption{Helmholtz coil structure. Further reference to direction will be in terms of the $\parallel$ direction, the $xy$ plane in the figure, and the $\perp$ direction, $\uz$ in the figure.}
    \label{diagram:coils}
  \end{figure}
...
\section{Methods}
\subsection{Coils}
Two 200 turn, $\qty{10}{\centi\meter}$ radius, Pasco Scientific Helmholtz coils were used \cite{pascoHelmholtzCoils}. These were mounted on a ruled base, also manufactured by Pasco Scientific. A DC current of approximately (not exceeding) $\qty{1.5}{\ampere}$ was passed through the coils in series with the coils connected such that the field produced by each was in the same direction.
...
In order to measure the field as a function of position in the coil a Pasco Scientific rotary motion sensor \cite{pascoPASPORTRotary} was used, with the magnetic field probe mounted to a rigid support whose extension into the coils could be tracked by the motion sensor.  Both the motion sensor and magnetic field probe were connected, via Pasco Scientific USB interfaces, to a desktop computer running the Pasco Capstone software for data collection.
\subsection{Alignment}
A challenge in configuring the experiment was the need to measure as closely as possible to 
the axis of symmetry of the coils, as this is the region in which the theory derived previously applies. To align the probe vertically two rulers were placed perpendicularly and the axial field indicator dot on the probe was aligned with the appropriate position on the rulers. To align horizontally a similar process was used except with the perpendicular field indicator dot. The motion sensor was zeroed at the midpoint between the coils. 
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}[]
        ...
        % Coils
        \draw[orange, ultra thick] (-\R/2,\R/2) -- (\R/2,\R/2) (-\R/2,-\R/2) -- (\R/2,-\R/2);

        % Axis
        \draw[->] (0,\R/2+\R/8) -- (\R/4,\R/2+\R/8) node[right]{$\parallel$};
        \draw[->] (0,\R/2+\R/8) -- (0,\R/2+\R/8+\R/4) node[above]{$\perp$};

        % Field probe
        \draw[fill=green!30!white] (-1/4,-\R/2-\R/4) rectangle (1/4,-\R/2-\R/36);
        \draw[-o] (0, -\R/2-\R/36) -- (0,-\R/2+\R/8) node[fill=white,rounded corners=2pt,inner sep=1pt, above]{Field Probe};
        \draw[->] (0,-\R/2-\R/4) to[out=270, in=180] (1/4+1,-\R/2-\R/2) node[right]{To Computer};

        % Linear motion sensor
        \fill[] (-1/3,-\R/2-\R/7) rectangle (1/3,-\R/2-\R/8);
        \draw[]
        (-1/3,-71*\R/112) -- (-1/2,-71*\R/112)
        -- (-1/2,-\R)
        ;
        \draw[fill=blue!30!white] (-1/2-1/3,-\R) rectangle (-1/2+1/3,-\R-\R/4) coordinate (A) ++ (-1/3,0)  node[below]{Motion Sensor};
        \draw[] (A) ++(0,\R/8) to[out=0, in=180] (1/4+1,-\R/2-\R/2);

        % Power supply & connections
        \draw[fill=red!30!white] (-\R+1,-\R/6) rectangle (-\R+1+\R/8,\R/6) (-\R+1,0) node[above, rotate=90]{Power Supply}
        (-\R+1+\R/16, \R/6-\R/24) coordinate(pterm) node[]{$+$}
        (-\R+1+\R/16, -\R/6+\R/24) coordinate(nterm) node[]{$-$}
        ;
        \draw[<-] (nterm) ++ (\R/16,0) to[out=0, in=0] (\R/2,-\R/2);
        \draw[->] (pterm) ++ (\R/16,0) to[out=0, in=180] (-\R/2,\R/2);
        \draw[->] (\R/2,\R/2) to[out=0, in=180] (-\R/2,-\R/2);
        \draw [thick, blue, ->]
        (-\R/8,\R/2) -- (\R/8,\R/2) node[midway, below] {$I$}
        (-\R/8,-\R/2) -- (\R/8,-\R/2) node[midway, above right] {$I$};

        \draw[-o] (0, -\R/2-\R/36) -- (0,-\R/2+\R/8);
    \end{tikzpicture}
    \caption{A cartoon of the experimental setup.}
    \label{diagram:coils}
  \end{figure}
\clearpage
\section{Results}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[mark=none, domain=-0.05:0.05] {((1.256*10^(-6))*1.5*200*(0.1)^2/2)*((1/((x-0.1/2)^2+0.1^2)^(1.5))+(1/((x+0.1/2)^2+0.1^2)^(1.5)))*10000};
...
\clearpage
\subsection{Off-Axis}
Off axis field measurements were conducted at increasing (towards coil center) distance from the edge of the coils.
\begin{figure}[h]
    \centering
\begin{tikzpicture}
  \begin{axis}[
  ...
  \end{axis}
\end{tikzpicture}
    \caption{Axial field at varying distance from coil edge.}
    \label{fig:helmholtz-3d-ax-field-data}
\end{figure}
\begin{figure}[h]
    \centering
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addlegendentry{$\qty{1.5}{\centi\meter}$}
    \addplot3[only marks, color=green!80!black] table [x=pos, y=fakepos, z=perp, col sep=comma] {run7-out.csv};
    \addlegendentry{$\qty{3}{\centi\meter}$}
    \addplot3[only marks, color=red!80!black] table [x=pos, y=fakepos, z=perp, col sep=comma] {run8-out.csv};
    \addlegendentry{$\qty{5}{\centi\meter}$}
    \addplot3[only marks, color=black] table [x=pos, y=fakepos, z=perp, col sep=comma] {run9-out.csv};
    \addlegendentry{$\qty{8}{\centi\meter}$}
  \end{axis}
\end{tikzpicture}
    \caption{Perpendicular field at varying distance from coil edge.}
    \label{fig:helmholtz-3d-perp-field-data}
\end{figure}
\section{Discussion}
\subsection{On-Axis}
As can be seen in \cref{fig:helmholtz-field-data}, the developed theoretical model agrees somewhat with the collected data. The model estimates a higher overall axial field in the uniform and non-uniform regions. Additionally there is a noticeable horizontal shift between the model and data. This is likely due to inaccurate zeroing of the linear motion sensor when collecting data. Shifting the data by approximately $\qty{2}{\centi\meter}$ to the left provides much better alignment with the theoretical model. The collected perpendicular data agrees well with the assumption that the perpendicular field is very small, approximately an order of magnitude smaller than the axial field.

In \cref{fig:helmholtz-2r-field-data} and \cref{fig:helmholtz-r/2-field-data} we see the expected nonuniformity of an improperly constructed Helmholtz coil system. We also see more variation in the perpendicular field. These figures both show the real accuracy of the model much better in the horizontal alignment. Interestingly they also show a much more similar magnetic field magnitude when compared to \cref{fig:helmholtz-field-data}. This points either to there being some effect which causes a field strength increase to match the model when in a non Helmholtz configuration or that the data in \cref{fig:helmholtz-field-data} is inaccurate. It is more likely that the data is inaccurate, perhaps due to a lower current (the theoretical curves assume exactly $\qty{1.5}{\ampere}$).
\subsection{Off-Axis}
The off-axis measurements do not have a theoretical result to compare against, though \cref{fig:helmholtz-3d-perp-field-data} does show the intuitively expected increase in perpendicular field strength due to closer proximity to one side of the coils. \cref{fig:helmholtz-3d-ax-field-data} also shows a similar effect, again due to the proximity to one side of the coils removing the ``balancing'' of the opposite side of the coils. The effect becomes less present closer to the center of the coils as expected, quickly converging to a highly uniform field.
